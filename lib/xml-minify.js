...
str.on('progress', function(progress) {
  ...
  const readStream = fs.createReadStream(inputPath).pipe(str);
  const xmlIn = new XmlStream(readStream);

  xmlIn.preserve('root products product', false);

  var rootNodePaths = whitelists[0].filterNode.split(' ');
  whitelists[0].filterNode = _.last(rootNodePaths);

  xmlIn.on('updateElement: product', function (baseNode) {
    try {
